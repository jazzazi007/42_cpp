...
void PhoneBook::add_contact(std::string data[5])
{
    int index;
    index = next_idx % 8;
    contact[index].set_firstname(data[0]);
    contact[index].set_lastname(data[1]);
    contact[index].set_nickname(data[2]);
    contact[index].set_number(data[3]);
    contact[index].set_secret(data[4]);
    this -> next_idx = this->next_idx + 1;
...
void	PhoneBook::display_contact(int i) const
{
	...
	std::cout << "Number: " << this->contact[i].get_number() << std::endl;
...
void	PhoneBook::display_phonebook() const
{
	...
	while (i < this->size)
	{
		...
		std::cout << i;
		std::cout << c_pre << "|" << c_pos;
		print_name(this->contact[i].get_firstname());
		std::cout << c_pre << "|" << c_pos;
		print_name(this->contact[i].get_lastname());
		std::cout << c_pre << "|" << c_pos;
		print_name(this->contact[i].get_nickname());
